...
def php_file():
    php_file_path = './templates/appointment.php'
    php_process = subprocess.Popen(
        ['php', php_file_path], stdout=subprocess.PIPE)
    php_output = php_process.communicate()[0]
    return php_output


@app.route('/login')
def php_file1():
    return redirect('http://localhost:8080/diagnosify/login.php')


@app.route('/register')
def php_file2():
    return redirect('http://localhost:8080/diagnosify/signup.php')


os.environ['_BARD_API_KEY'] = 'XQhlefBH7fsp8zMvISXbIWAE8QS8POeFjL4LbweNQPZ5Q9jiqj7_MtXVIoKjw-H1fdcYGw.'
...
def search():
    ...
    if query:
        ...
        response = {'content': 'No query provided.'}
    return response
...
if __name__ == '__main__':
    app.run(debug=True, ssl_context='adhoc', port=8080, host='localhost')
